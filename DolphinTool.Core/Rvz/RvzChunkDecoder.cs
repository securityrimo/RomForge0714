...
internal sealed class RvzChunkDecoder : IDisposable
{
    ...
    private readonly RvzDecompressor _decompressor;
    private readonly RvzCompressionType _compression;
    private readonly LaggedFibonacciGenerator _generator = new();
    private readonly DecodedChunk _chunk = new();
    private byte[] _scratch = [];

    public RvzChunkDecoder(RvzCompressionType compression)
    {
        _compression = compression;
        _decompressor = RvzDecompressor.Create(compression);
    }

    public DecodedChunk Decode(ReadOnlySpan<byte> input, bool compressedFlag, int exceptionLists, int dataSize,
        uint rvzPackedSize, long junkOffset)
    {
        bool compressed = compressedFlag && _compression != RvzCompressionType.None;
